...
def parse_arguments():
    ...
    parser.add_argument("--ea", action="store_true", default=False,
                        help="Enable Euclidean Alignment (EA) per-subject preprocessing")
    parser.add_argument("--max_epochs_override", type=int, default=None,
                        help="Override max_epochs for smoke testing (does not modify config file)")
...
def run():
    ...
    # ── Euclidean Alignment ──
    config["preprocessing"]["use_ea"] = args.ea
    print(f"[Config] use_ea = {args.ea}")
    ...
    # ── Max epochs override (for smoke testing) ──
    if args.max_epochs_override is not None:
        config["max_epochs"] = args.max_epochs_override
        print(f"[Config] max_epochs overridden to {args.max_epochs_override} (smoke test)")
